...
- **内置专家角色**：提供 `analyzer`、`architect`、`reviewer` 及其前端变体（`*-frontend`）角色提示词。
- **前端审查仍使用 Codex**：前端/UI 审查使用专用 Codex 角色提示词 `reviewer-frontend`，不引入模型路由。
...
Codex CLI
   ...
   +-- codeagent-wrapper
          |
          +-- Codex 子进程（analyzer / analyzer-frontend / architect / architect-frontend / reviewer / reviewer-frontend）
```
