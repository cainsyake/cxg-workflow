...
CXG 是一个面向 Codex CLI 的单模型工作流包。它会安装一组结构化 skills、角色提示词、子 Agent 模板和 `codeagent-wrapper` 二进制，使 Codex 能以统一方式完成研究、规划、执行、优化和评审。
...
- **skills-native 运行时**：`init` 会把 CXG skills、roles、子 Agent 模板和 wrapper 安装到 `~/.codex/skills/` 与 `~/.codex/.cxg/`。
- **结构化交付**：主工作流固定为 `研究 -> 构思 -> 计划 -> 执行 -> 优化 -> 评审`。
- **内置专家角色**：提供 `analyzer`、`architect`、`debugger`、`optimizer`、`reviewer`、`tester`，并补充前端专项角色（`analyzer-frontend`、`architect-frontend`、`debugger-frontend`、`frontend`、`optimizer-frontend`、`reviewer-frontend`、`tester-frontend`）。
- **预置子 Agent 模板**：内置 `get-current-datetime`、`init-architect`、`planner`、`ui-ux-designer`，用于 `cxg-init`/`cxg-feat` 协同。
- **前端工作流仍使用 Codex**：前端开发/调试/优化/测试/审查均使用 Codex 专用角色提示词，不引入模型路由。
- **可选代码检索**：支持 `ace-tool`，可预留 `contextweaver`，不配置 MCP 时自动退回 `Glob + Grep`。
- **直接可用的技能入口**：安装后即可通过 12 个 `$cxg-*` 入口驱动工作流。

## 同步与差异记录

- [从 ccg-workflow 同步与差异记录](./SYNC-FROM-CCG.zh-CN.md)
...
Codex CLI
   ...
   +-- codeagent-wrapper
          |
          +-- Codex 子进程（analyzer / analyzer-frontend / architect / architect-frontend / debugger / debugger-frontend / frontend / optimizer / optimizer-frontend / reviewer / reviewer-frontend / tester / tester-frontend）
```
...
| Codex CLI | 是 | CXG 会把 skills 与 `.cxg` 运行时资产安装到 `~/.codex/` 供 Codex CLI 使用 |
...
安装完成后，在 Codex CLI 中通过 skills 入口调用：
...
- `templates/commands/agents/` 包含从 `ccg-workflow/templates/commands/agents` 迁入的子 Agent 提示词模板。
- `templates/prompts/` 会继续保留在仓库里，作为 prompt 时代的迁移/参考资料，而不是运行时入口。
- `cxg-workflow init` 会将其安装到 `~/.codex/skills/cxg/`。
- `cxg-workflow init` 会将子 Agent 模板安装到 `~/.codex/.cxg/agents/codex/`。
...
### Codex CLI 里看不到 CXG skills

先执行 `npx cxg-workflow init`，然后重启 Codex CLI。CXG 会把运行时 skill 资产安装到 `~/.codex/skills/cxg/`，并把角色与子 Agent 资产放到 `~/.codex/.cxg/` 下，重启后 Codex 会重新加载这些内容。
