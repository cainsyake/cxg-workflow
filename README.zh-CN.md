...
CXG 是一个面向 Codex CLI 的单模型工作流包。它会安装一组结构化 skills、角色模板和 `codeagent-wrapper` 二进制，使 Codex 能以统一方式完成研究、规划、执行、优化和评审。
...
- **Skill-only 运行时**：`init` 会安装 skills、roles 和 wrapper。
...
- **内置子agent提示词模板**：预置 `get-current-datetime`、`init-architect`、`planner`、`ui-ux-designer`，可被 `$cxg-init`、`$cxg-feat` 等工作流复用。
- **前端工作流仍使用 Codex**：前端开发/调试/优化/测试/审查均使用 Codex 专用角色提示词，不引入模型路由。
- **可选代码检索**：支持 `ace-tool`，可预留 `contextweaver`，不配置 MCP 时自动退回 `Glob + Grep`。
- **直接可用的工作流技能**：安装后会把 12 个命令技能写入 `~/.codex/skills/cxg/`。
...
- `templates/agents/codex/` 提供可复用的子agent提示词模板（由 `codeagent-wrapper` 调用）。
- `cxg-workflow init` 会将这些资产安装到 `~/.codex/skills/cxg/` 与 `~/.codex/.cxg/agents/codex/`。
...
- 技能模板内部调用 `mcp__ace-tool__search_context`
...
### Codex CLI 里看不到 CXG 技能

先执行 `npx cxg-workflow init`，然后重启 Codex CLI。CXG 会把技能文件写入 `~/.codex/skills/cxg/`。
