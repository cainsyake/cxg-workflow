...
CXG 是一个面向 Codex CLI 的单模型工作流包。它会安装一组结构化 Slash Commands、skills、角色提示词、子 Agent 模板和 `codeagent-wrapper` 二进制，使 Codex 能以统一方式完成研究、规划、执行、优化和评审。
...
- **运行时 prompts + skills**：`init` 会安装 prompts、迁移后的 skills、roles、子 Agent 模板和 wrapper。
