...
`src/` contains the TypeScript source for CLI entrypoints, commands, shared utilities, and exported types. Use `src/commands/` for user-facing actions, `src/utils/` for reusable logic, and `src/types/` for shared interfaces. Role and skill templates live in `templates/roles/codex/` and `templates/skills/cxg/`. Treat `dist/` as generated output.
