...
CXG is a Codex-first workflow package for Codex CLI. It installs a structured set of skills, role templates, and a `codeagent-wrapper` binary so Codex can handle research, planning, execution, optimization, and review in a consistent single-model workflow.
...
After installation, open Codex CLI and invoke skills such as:
...
Run `npx cxg-workflow init`, then restart Codex CLI. CXG installs skill files into `~/.codex/skills/cxg/`.
