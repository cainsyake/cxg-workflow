...
function getSkillsDir() {
  ...
  const allowedBase = resolve(homedir(), '.codex', 'skills', 'cxg');
  const inAllowedBase = resolved === allowedBase || resolved.startsWith(`${allowedBase}/`) || resolved.startsWith(`${allowedBase}\\`);

  if (!inAllowedBase) {
    console.warn(`警告: 忽略不安全的 SAGE_SKILLS_DIR=${resolved}`);
    return defaultDir;
  }
