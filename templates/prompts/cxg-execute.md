...
- **写入策略**：默认输出 Unified Diff Patch。仅在用户显式授权后，才允许子进程写入批准计划中的白名单文件。
- **范围校验**：每次直接写入前必须回显白名单并确认；写入后必须校验改动文件全部落在白名单内。
...
优先输出 `Unified Diff Patch`；若已显式授权，可直接写入白名单文件并输出变更摘要（写入前回显白名单）。
...
3. 最小作用域落盘，避免非需求副作用
4. 校验改动文件全部在白名单内；若越界，立即停止并报告用户
5. 运行最小必要验证（lint/typecheck/tests）
...
1. **白名单写入** - 写入前回显并确认白名单，子进程仅允许写入批准计划中的目标文件
