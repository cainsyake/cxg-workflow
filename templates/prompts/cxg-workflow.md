...
- 要开发的任务：$ARGUMENTS
- 带质量把关的结构化 6 阶段工作流
- 单模型多子进程协作：统一使用 Codex 模型，主 Codex 负责编排并按任务类型路由子进程
...
**按需调用**：根据任务类型与修改范围决定调用后端、前端或双子进程；已启动任务统一使用 `run_in_background: true` 并等待返回。**必须等所有已启动模型返回后才能进入下一阶段**。
...
- ⛔ **Codex 结果必须等待**：仅当已启动 Codex 子进程任务时，本规则生效。Codex 执行时间较长（5-15 分钟）属于正常。单次查询超时后必须继续轮询后台结果，**绝对禁止在 Codex 未返回结果时直接跳过或继续下一阶段**。已启动的 Codex 任务若被跳过 = 浪费 token + 丢失结果。
...
按任务类型与修改范围调用：
- 后端 + 单域：仅调用 Codex 后端分析（技术可行性、方案、风险）
- 前端 + 单域：仅调用 Codex 前端分析（UI 可行性、方案、体验）
- 全栈或跨域：并行调用后端 + 前端分析

等待已启动任务结果。**📌 保存 SESSION_ID**（按实际启动记录：`CODEX_SESSION` 和/或 `CODEX_FRONTEND_SESSION`）。

**务必遵循上方 `子进程调用规范` 的 `重要` 指示**

综合已获取分析结果，输出方案对比（至少 2 个方案），等待用户选择。

### 📋 阶段 3：详细规划

`[模式：计划]` - 子进程按需协作规划：

按阶段 1/2 的路由结果复用会话 `resume <SESSION_ID>`：
- 后端路由：使用规划提示词 + `resume <<CODEX_SESSION>>`，输出后端架构
- 前端路由：使用规划提示词 + `resume <<CODEX_FRONTEND_SESSION>>`，输出前端架构
- 全栈/跨域：并行复用两路会话输出联合架构
...
**Codex 主进程**：采纳已调用子进程规划结果（单路或双路），保存计划文件： `.codex/plan/任务名.md`；
保存完计划文件后询问用户：

- **开始实施** → 阶段 4
- **讨论调整** → 结合用户的反馈调整，重新执行阶段 3
- **重新规划** → 删除当前计划，重新执行阶段 3
- **仅保存计划** → 退出
...
1. 阶段顺序不可跳过（除非用户明确指令）
2. 子进程调用必须按任务类型与修改范围路由；仅在全栈或跨域场景并行调用前后端子进程，单域场景禁止机械性双调用
3. 评分 <7 或用户未批准时**强制停止**
